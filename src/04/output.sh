...
print_color_scheme() {
    ...
    for i in {0..3}; do
        param_name=""
        case $i in
            0) param_name="Column 1 background";;
            1) param_name="Column 1 font color";;
            2) param_name="Column 2 background";;
            3) param_name="Column 2 font color";;
        esac
        
        if [[ "${result[$i]}" == "${defaults[$i]}" ]]; then
            echo "$param_name = default (${color_names[${defaults[$i]}]})"
        else
            echo "$param_name = ${result[$i]} (${color_names[${result[$i]}]})"
        fi
    done
}